...
export default function App() {
    ...
    // <--- ЗМІНЕНО: Логіка сортування та фільтрації (Ваш варіант)
    const filtered = useMemo(() => {
        let a = files.slice();

        // Фільтрація (Варіант 7: C, JPG)
        if (filter === 'C') a = a.filter(m => m.name.toLowerCase().endsWith('.c'))
        if (filter === 'JPG') a = a.filter(m => m.name.toLowerCase().endsWith('.jpg') || m.name.toLowerCase().endsWith('.jpeg'))

        // Сортування (Варіант 7: за датою створення)
        a.sort((x, y) => {
            const dx = new Date(x.createdAt).getTime();
            const dy = new Date(y.createdAt).getTime();
            return (sortAsc ? 1 : -1) * (dx - dy);
        })
        return a;
...
function Toolbar({ filter, setFilter, sortAsc, setSortAsc, hiddenCols, setHidden, onUpload, selected, onDelete }: {
    ...
    return (
        <div style={{ display: 'flex', gap: 8, padding: 8, alignItems: 'center', borderBottom: '1px solid #ddd' }}>
            ...
            {/* <--- ЗМІНЕНО: Опції фільтра (Варіант 7: C, JPG) */}
            <select value={filter} onChange={e => setFilter(e.target.value as Filter)}>
                <option value='ALL'>All</option>
                <option value='C'>.c</option>
                <option value='JPG'>.jpg</option>
            </select>
            {/* <--- ЗМІНЕНО: Назви кнопок сортування (Варіант 7: за датою створення) */}
            <button onClick={() => setSortAsc(true)}>Created ↑</button>
            <button onClick={() => setSortAsc(false)}>Created ↓</button>
...
// <--- ЗМІНЕНО: Логіка компонента Preview (Ваш варіант)
function Preview({ meta }: { meta: Meta | null }) {
    ...
    // (Варіант 8: .jpg)
    if (ext === 'jpg' || ext === 'jpeg') {
        return <div style={{ padding: 12, border: '1px solid #eee' }}><img src={downloadUrl(meta.id)} style={{ maxWidth: '100%' }} /></div>
    }
    ...
    return <div style={{ padding: 12, border: '1px solid #eee' }}>No preview available</div>
}

// ---- Sync (File System Access API) with anti-echo index + two-way delete ----
// <--- ПОВЕРНУЛИ ВЕСЬ ВІДСУТНІЙ КОД ДЛЯ СИНХРОНІЗАЦІЇ
